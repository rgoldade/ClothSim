...
void Renderer::addTriangles(const VecVec3d& vertices, const VecVec3d& vertexNormals,
                           const VecVec3i& faces, const Vec3d& faceColour)
{
    assert(vertices.size() == vertexNormals.size());

    myTriVertices.push_back(vertices);
    myTriVertexNormals.push_back(vertexNormals);
...
void Renderer::drawPrimitives() const
{
    ...
    {
        Vec3f pointColour = myPointColours[pointListIndex].cast<float>();
        ...
        {
            Vec3f point = myPoints[pointListIndex][pointIndex].cast<float>();
    ...
    {
        Vec3f lineColour = myLineColours[lineListIndex].cast<float>();
        ...
        {
            Vec3f startPoint = myLineStartingPoints[lineListIndex][lineIndex].cast<float>();
            glVertex3f(startPoint[0], startPoint[1], startPoint[2]);

            Vec3f endPoint = myLineEndingPoints[lineListIndex][lineIndex].cast<float>();
    ...
    assert(myTriVertices.size() == myTriVertexNormals.size() && myTriVertices.size() == myTriFaces.size() &&
           myTriVertices.size() == myTriFaceColours.size());

    for (int triListIndex = 0; triListIndex < int(myTriFaceColours.size()); ++triListIndex)
    {
        Vec3f triFaceColour = myTriFaceColours[triListIndex].cast<float>();
        glColor3f(triFaceColour[0], triFaceColour[1], triFaceColour[2]);

        glBegin(GL_TRIANGLES);

        assert(myTriVertices[triListIndex].size() == myTriVertexNormals[triListIndex].size());

        for (int triIndex = 0; triIndex < int(myTriFaces[triListIndex].size()); ++triIndex)
        {
            for (int pointIndex : {0, 1, 2})
            {
                ...
                Vec3f vertexPoint = myTriVertices[triListIndex][trianglePointIndex].cast<float>();
                glVertex3f(vertexPoint[0], vertexPoint[1], vertexPoint[2]);

                Vec3f normal = myTriVertexNormals[triListIndex][trianglePointIndex].cast<float>();
...
void Renderer::setGenericLights(void) const
{
    glEnable(GL_LIGHTING);
    {
        GLfloat ambient[4] = {.3f, .3f, .3f, 1.f};
